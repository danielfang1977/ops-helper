...
func newAllowTrafficCmd() *cobra.Command {
	command := &cobra.Command{
		Use:   "allow-traffic",
		Short: "register instance to target group",
		Long:  `example: ops-helper aws allow-traffic -p dev -i instanceID`,
			...
			if size > 0 {
				...
				for _, t := range targetGroups {
					go func(targetGroupArn *string) {
						defer wg.Done()
						log.Printf("start register instanceID[%s], arn[%s]\n", inputInstanceID, *targetGroupArn)
						err := albSvc.Register(&inputInstanceID, targetGroupArn)
						if err != nil {
							panic(err)
						}
						log.Printf("end register instanceID[%s], arn[%s]\n", inputInstanceID, *targetGroupArn)
					}(t.Arn)
				}
				wg.Wait()
			}
